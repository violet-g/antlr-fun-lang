...
public class SVM {
	...
	//    5    1    LOADR      pop upper; pop lower; pop element                   // EXTENSION
	//                         push (if element is in range then 1 else 0).
	...
	//   11		 1	  COMPG      pop guard; retrieve element;
	//                         push(if gurad=element then 1 else 0).      					// EXTENSION
	...
	// as it is executed.
		...
		do {
			...
			switch (opcode) {
				...
				case LOADR: { // load range                                                 // EXTENSION
					int upper = data[--sp];
					int lower = data[--sp];
					int e = data[--sp];
					data[sp++] = (((e>=lower) && (e<=upper)) ? 1: 0);
					break;
				}
				case ADD: {
					int w2 = data[--sp];
				...
				case COMPG: { //compare guard                                             // EXTENSION
					int guard = data[--sp];
					int e = data[sp-1];
					data[sp++] = (guard == e ? 1 : 0);
					break;
