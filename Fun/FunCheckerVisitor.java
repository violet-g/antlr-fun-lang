...
public class FunCheckerVisitor extends AbstractParseTreeVisitor<Type> implements FunVisitor<Type> {
	...
	public Type visitSwitch(FunParser.SwitchContext ctx) {                                //EXTENSION
		...
		if (expr_type != Type.BOOL && expr_type != Type.INT) {
			reportError("incorrect type of tested expression", ctx);
		}
		List<FunParser.GuardContext> guards = ctx.guard();
		List<String> visitedGuards = new ArrayList<String>();

		for (FunParser.GuardContext guard : guards){
			Type guard_type = visit(guard);
			checkType(guard_type, expr_type, ctx);
			if (visitedGuards.contains(guard.getText())){
				reportError("repeated or overlapping guards", ctx);
			} else if (guard_type == Type.BOOL) {
				checkType(guard_type, expr_type, ctx);
				visitedGuards.add(guard.getText());
			} else {
				if (guard.getText().matches("\\d+")) {
				checkType(guard_type, expr_type, ctx);
				visitedGuards.add(guard.getText());
				} else {
					String[] range_line = (guard.getText()).split("\\..");
					int i = Integer.parseInt(range_line[0].replaceAll("[\\D]", ""));
					int j = Integer.parseInt(range_line[1].replaceAll("[\\D]", ""));
					if (i >= j) {
						reportError("first value in range should be smaller", ctx);
					}
					for (String value : visitedGuards) {
						if (i <= Integer.parseInt(value) && j >= Integer.parseInt(value)){
							reportError("repeated or overlapping guards", ctx);
						}
					}
					for (int num = i; num <= j; num++){
						visitedGuards.add(Integer.toString(num));
					}
				}
			}
			visit(ctx.c1);
	 ...
	 * @return the visitor result
	 */
	public Type visitFalseg(FunParser.FalsegContext ctx) {                                  // EXTENSION
		return Type.BOOL;
	}
	 ...
	 * @return the visitor result
	 */
	public Type visitTrueg(FunParser.TruegContext ctx) {                                    // EXTENSION
		return Type.BOOL;
	}
	 ...
	 * @return the visitor result
	 */
	public Type visitNumg(FunParser.NumgContext ctx) {                                      // EXTENSION
		return Type.INT;
	}
	 ...
	 * @return the visitor result
	 */
	public Type visitRangeg(FunParser.RangegContext ctx) {																// EXTENSION
		return Type.INT;
	}
}
